...
struct HelpOption final : public IOptionHandler
{
    std::string getName() { return "help"; }

    void handle(const std::string &value) { std::cout << "help commands ..." << std::endl; }
};

struct InfoOption final : public IOptionHandler
{
    std::string getName() { return "info"; }

    void handle(const std::string &value) { std::cout << "some info ..." << std::endl; }
};

struct NumberOption final : public IOptionHandler
{
    std::string getName() { return "number"; }

    void handle(const std::string &value) { std::cout << "number is: " << std::stoi(value) << std::endl; }
};
...
class CliHandler
{
  ...
  public:
    CliHandler(std::vector<IOptionHandler *> optionHandlers)
    {
        for (auto handler : optionHandlers)
        {
  ...
  private:
    std::pair<std::string, std::string> parseOption(std::string option)
    {
        int idx = eatDashes(option);
        return {getName(idx, option), getValue(idx, option)};
    }

    int eatDashes(const std::string &option)
    {
    ...
    std::string getName(int &index, const std::string &option)
    {
        int begin = index;
    ...
    std::string getValue(int &index, const std::string &option)
    {
...
class Application
{
  ...
  public:
    Application(CliHandler *cliHandler) : _cliHandler(*cliHandler) {}
