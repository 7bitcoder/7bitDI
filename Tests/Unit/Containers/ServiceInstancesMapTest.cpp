...
TEST_F(ServiceInstancesMapTest, ShouldInsert)
{
    ...
    auto act = [&] {
        map.insert(typeid(TestClass1), sb::di::ServiceInstance{sb::di::ServiceInstance{std::move(instance)}});
    };
...
TEST_F(ServiceInstancesMapTest, ShouldCheckEmpty)
{
    ...
    map.insert(typeid(TestInheritClass1), sb::di::ServiceInstance{sb::di::ServiceInstance{std::move(instance)}})
        .add(sb::di::ServiceInstance{sb::di::ServiceInstance{std::move(instance2)}});
...
TEST_F(ServiceInstancesMapTest, ShouldContainsList)
{
    ...
    TestInheritClass2 test2;
    sb::di::IServiceInstance::Ptr instance2{new sb::di::details::services::ExternalService{&test2}};
    map.insert(typeid(TestInheritClass1), sb::di::ServiceInstance{std::move(instance)})
        .add(sb::di::ServiceInstance{std::move(instance2)});
...
TEST_F(ServiceInstancesMapTest, ShouldFindList)
{
    ...
    EXPECT_EQ(list->first().getImplementation().getTypeId(), typeid(TestInheritClass3));
    EXPECT_TRUE(list->last().isValid());
    EXPECT_EQ(list->last().getAs<void>(), &test2);
    EXPECT_EQ(list->last().getImplementation().getTypeId(), typeid(TestInheritClass2));
}
